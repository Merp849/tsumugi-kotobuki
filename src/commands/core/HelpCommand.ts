...
export default class HelpCommand extends BaseCommand {
    run(message: IMessage): Promise<IMessage> {
        if (!message.args[0]) {
            ...
            if (!cmd) return message.channel.send(`\`${message.args[0]}\` doesn't exists in my commands list! Perhaps a typo?`);
            const usage = cmd.meta!.usage ? `Don't know how to use this command? Easy, just use \`${this.client.config.prefix}${cmd.meta!.usage}`: "No usage provided for this command.";
            const example = cmd.meta!.example ? `Still don't know? Okay, here the example: \`${this.client.config.prefix}${cmd.meta!.example}\`` : "No example provided for this command.";
